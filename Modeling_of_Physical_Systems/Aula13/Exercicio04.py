...
F = np.zeros(len(t[it[0]:]))
for nf in range(5):
    an, bn = abfourier(t, x, it[0], it[1], nf)
    F += an*np.cos(nf*t[it[0]:]) + bn*np.sin(nf*t[it[0]:])
    
print("c)")
#print(F)

plt.figure()
plt.plot(t[it[0]:], x[it[0]:], label='result')
plt.plot(t[it[0]:], F, label='fourier')
plt.legend()
plt.grid()
plt.show()
