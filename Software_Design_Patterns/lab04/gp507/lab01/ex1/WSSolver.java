...
public class WSSolver {
    ...
    public static boolean hasEmptyLines(File f) throws Exception {
    ...
    private static void createMatrix(File f) throws Exception {
        ...
        while (sc.hasNextLine() && line.matches("[a-zA-Z]*") && line.length() == puzzleSize) { // Stop reading when it reaches the words to find
        ...
        if (n_cols != puzzleSize || n_cols != n_rows) {
        ...
        while (sc.hasNextLine()) {
            ...
            if (!line.matches("[A-Z]*")) {
                for (String word : line.split("[\\s,;]+")) {
                    if (word.matches("[a-zA-Z]*")) { // Must be only aphabetical characters
                        if (word.toUpperCase().equals(word)) { // Must not be all Uppercase
                            sc.close();
                            throw new Exception("List words must be lower case or mixed");
                        }
                        else if(word.length() > puzzle.length){
                            sc.close();
                            throw new Exception("Word doesn't fit the puzzle");
                        } 
                        else
                            words.add(word);
                    } else {
                        sc.close();
                        throw new Exception("The list of words must only contain alphabetical characters.");
                    }
                }
            }
        }
    ...
    private static void searchWords() throws Exception {
        for (String word : words) {
            if (!searchWord(word)) {
                String excep = "Word \"" + word + "\" is not in the puzzle.";
